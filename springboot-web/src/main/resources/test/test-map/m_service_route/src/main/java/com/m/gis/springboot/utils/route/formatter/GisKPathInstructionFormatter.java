 ...
 * @Version: V1.0
 */
public class GisKPathInstructionFormatter implements GisRouteFormatter<GisShortestPathVO> {
    private Locale locale;

    public GisKPathInstructionFormatter(Locale locale) {
        this.locale = locale;
    }
    ...
    public GisShortestPathVO format(List<List<GisRouteRoad>> gisRouteRoadGroups) {
        ...
        for(int i=0;i<gisRouteRoadGroups.size();i++){
            ...
            GisPathVO gisPathVO = GisLegStepHandleClient.handle(1,gisRouteRoads,locale);
